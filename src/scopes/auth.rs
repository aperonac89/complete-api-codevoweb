...
pub async fn register(
    ...
    let hashed_password = password::hash(&body.password).map_err(|e| HttpError::server_error(e.to_string()))?;
    ...
    match result {
        ...
        Err(sqlx::Error::Database(db_err)) => {
            if db_err.is_unique_violation() {
                Err(HttpError::unique_constrains_violation(
...
pub async fn login(
    ...
    let password_matches = password::compare(&body.password, &user.password)
        .map_err(|e| HttpError::unauthorized(ErrorMessage::WrongCredentials))?;
    ...
    if password_matches {
        ...
        .map_err(|e| HttpError::serber_error(e.to_string()))?;
